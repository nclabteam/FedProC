...
class _EncoderBlock(nn.Module):
    def __init__(self, d_model, mha, d_hidden, dropout=0.0, channel_wise=False):
        ...
        if channel_wise:
            self.conv = nn.Conv1d(d_model, d_model, kernel_size=1, padding_mode="reflect")
        self.mha = mha
...
class MultiPatchFormer(nn.Module):
    ...
    def __init__(self, configs):
        ...
        shared_mha = nn.ModuleList([AttentionLayer(FullAttention(mask_flag=True), d_model, n_heads) for _ in range(N)])
        shared_mha_ch = nn.ModuleList([AttentionLayer(FullAttention(mask_flag=True), d_model, n_heads) for _ in range(N)])

        self.enc_layers = nn.ModuleList(
            [_EncoderBlock(d_model, shared_mha[i], d_hidden, dropout, channel_wise=False) for i in range(N)]
        )
        self.enc_layers_ch = nn.ModuleList(
            [_EncoderBlock(d_model, shared_mha_ch[0], d_hidden, dropout, channel_wise=True) for _ in range(N)]
        )
    ...
    def forward(self, x, **kwargs):
        ...
        enc = torch.cat([e1, e2, e3, e4], dim=-1) + self.pe  # [B*C, patch_num1, d_model]
        ...
        x_ch = self.embed_ch(x_patch_c.permute(0, 2, 1)).transpose(1, 2)  # [B, C, d_model]
        ...
        out = torch.cat([f1, f2, f3, f4, f5, f6, f7, f8], dim=-1).permute(0, 2, 1)  # [B, pred_len, C]
