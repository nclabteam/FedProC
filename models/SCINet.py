...
class SCIBlock(nn.Module):
    def __init__(self, enc_in, kernel_size=3, dilation=1, dropout=0.5, d_model=64):
        ...
        pad_l = (
            ...
            if kernel_size % 2 != 0
            else dilation * (kernel_size - 2) // 2 + 1
        )
        pad_r = (
            dilation * (kernel_size - 1) // 2 + 1
            if kernel_size % 2 != 0
            else dilation * (kernel_size) // 2 + 1
        )
...
class SCITree(nn.Module):
    def __init__(
        ...
        if level != 0:
            self.SCINet_odd = SCITree(
                level - 1, enc_in, kernel_size, dilation, dropout, d_model
            )
            self.SCINet_even = SCITree(
                level - 1, enc_in, kernel_size, dilation, dropout, d_model
            )
    ...
    def forward(self, x):
        ...
        if self.level == 0:
            ...
            return self.zip_up_the_pants(
                x.shape, self.SCINet_even(x_even_update), self.SCINet_odd(x_odd_update)
            )
