...
class MMK(nn.Module):
    ...
    def __init__(
        ...
        self.layer_hp = configs.layer_hp
        self.layer_type = configs.layer_type
        self.hidden_dim = configs.hidden_dim
        self.in_channels = configs.input_channels

        if self.layer_type == "MoK":
            if self.layer_num == 1:
                self.model = MoKLayer(self.hist_len, self.pred_len, self.layer_hp)
            elif self.layer_num == 2:
                self.model = nn.Sequential(
                    MoKLayer(self.hist_len, self.hidden_dim, self.layer_hp),
                    MoKLayer(self.hidden_dim, self.pred_len, self.layer_hp),
                )
                ...
                self.model = nn.Sequential(
                    ...
                    MoKLayer(
                        ...
                        self.hidden_dim,
                        self.layer_hp,
                        res_con=self.use_norm,
                        with_bn=self.use_norm,
                        with_dropout=self.use_norm,
                    ),
                    MoKLayer(self.hidden_dim, self.pred_len, self.layer_hp),
                )
            ...
            if self.layer_num == 1:
                self.model = nn.Linear(self.hist_len, self.pred_len, bias=False)
            else:
                self.model = nn.Sequential(
                    nn.Linear(self.hist_len, self.hidden_dim, bias=False),
                    nn.Linear(self.hidden_dim, self.hidden_dim, bias=False),
                    nn.Linear(self.hidden_dim, self.pred_len, bias=False),
                )
        else:
            if self.layer_num == 1:
                self.model = KANInterfaceV2(
                    self.hist_len,
                    self.pred_len,
                    layer_type=self.layer_type,
                    hyperparam=self.layer_hp,
                )
            else:
                self.model = nn.Sequential(
                    KANInterfaceV2(
...
class KANInterfaceV2(nn.Module):
    def __init__(self, in_features, out_features, layer_type, hyperparam):
        ...
        if layer_type == "KAN":
            ...
            self.transform = NaiveFourierKANLayer(
                in_features, out_features, gridsize=hyperparam
            )
        elif layer_type == "JacobiKAN":
            self.transform = JacobiKANLayer(
                in_features, out_features, degree=hyperparam
            )
...
class NaiveFourierKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = torch.einsum(
            "dbik,djik->bj", torch.concat([c, s], dim=0), self.fouriercoeffs
        )
