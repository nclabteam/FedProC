...
class S3(nn.Module):
    ...
    def __init__(self, configs):
        ...
        for i in range(0, configs.num_layers):
            ...
            self.S3_layers += [
                S3Layer(
                    num_segments=next_segment_num,
                    shuffle_vector_dim=configs.shuffle_vector_dim,
                    use_conv_w_avg=configs.use_conv_w_avg,
                    initialization_type=configs.initialization_type,
                    use_stitch=configs.use_stitch,
                )
            ]
            next_segment_num = int(next_segment_num * configs.segment_multiplier)
    ...
    def forward(self, x):
        ...
        if x.shape[1] > x_copy.shape[1]:
            x_copy = torch.cat(
                [x[:, 0 : x.shape[1] - x_copy.shape[1], :], x_copy], dim=1
            )
...
class S3Layer(nn.Module):
    def __init__(
        ...
        if self.use_conv_w_avg:
            ...
            self.weights = nn.Parameter(
                torch.ones(2) * 0.5
            )  # Initialize with two random values for w1, w2
    ...
    def initialize_shuffle_vector(self):
        ...
        if self.initialization_type == "kaiming" and self.shuffle_vector_dim > 1:
            ...
            init.kaiming_normal_(
                self.shuffle_vector, mode="fan_out", nonlinearity=self.activation
            )
    ...
    def forward(self, x):
        ...
        if len(self.shuffle_vector.shape) > 1:
            shuffle_vector_sum = self.shuffle_vector.sum(
                tuple(range(len(self.shuffle_vector.shape) - 1))
            )
        else:
        ...
        result_matrix = torch.zeros(
            (len(shuffle_vector_sum), len(shuffle_vector_sum)), device=x.device
        )
        ...
        result_matrix.scatter_(
            1, self.descending_indices.unsqueeze(1), shuffle_vector_sum.unsqueeze(1)
        )
        ...
        stacked_segments = stacked_segments.unsqueeze(-1).expand(
            -1, -1, -1, -1, stacked_segments.shape[-1]
        )
        ...
        if self.use_stitch:
            ...
            if self.use_conv_w_avg:
                ...
                stacked_shuffle_original_reshaped = stacked_shuffle_original.view(
                    b * t * c, 2, 1
                )
