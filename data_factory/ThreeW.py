...
class ThreeWBase(BaseDataset):
    ...
    def read(path: str) -> pl.DataFrame | None:
        """Read a 3W client while inferring sensor types from the full file."""
    ...
    def _list_instance_files(self, label: str) -> list[str]:
        response = requests.get(
            f"{self.repo_api}/{label}", timeout=self.request_timeout
        )
    ...
    def download(self) -> None:
        """Download and convert eligible 3W episodes to client CSV files."""
...
class ThreeWReal(ThreeWBase):
    """Real Petrobras 3W well episodes."""
...
class ThreeWSimulated(ThreeWBase):
    """Simulated Petrobras 3W well episodes."""
