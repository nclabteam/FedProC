...
† TSF adaptation applied: class-level assumptions replaced with time-series equivalents. Please use with caution.
  ...
  - **Caesar†**: KL-divergence term in importance estimation dropped (no class labels in TSF). Upload ranking uses participant subset only (server lacks sample counts for all clients), so ranks are compressed vs. the paper's global ranking. Uplink measured as COO sparse gradient (int64 index + float32 value per retained element); downlink measured as true compressed wire size (full-precision elements + 1-bit signs + per-tensor stats). Default hyperparameters from paper (Section 5.1): `theta_d_max=0.6`, `theta_u_min=0.1`, `theta_u_max=0.6`, `lambda=0.5`.

**Implementation notes** — non-TSF-adaptation deviations worth flagging (architecture constraints or unvalidated defaults, not a class→time-series swap):
  - **DeComFL**: Uplink is faithful — `q` scalar ZO-gradient estimates per client (dimension-free, ~76 bytes regardless of model size), matching the paper's forward-difference formula `g^k = (f(w+μu)-f(w))/μ` and revert-after-K-steps procedure (Algorithm 2). Downlink is **not** dimension-free like the paper: FedProC's stateless-client architecture cannot replay a client's local model history from seeds alone, so the full float32 global model is sent every round instead of the paper's seed-only reconstruction. `mu=0.001` matches the paper's default; `q=2` and `zo_lr=0.01` are TSF-adapted choices (paper sweeps `P` up to 10 and uses task-specific LRs from 5e-6–1e-3, none of which were tuned for TSF). This is left unfixed on purpose — reporting a smaller, seed-based downlink byte count without actually giving clients persistent state would desync the `downlink_mb` metric from `time_per_iter`, which still reflects the real cost of transmitting the full model every round.
  - **FedLUAR**: Exact match to Algorithm 1 and Eqs. 1–2 — layer selection via stochastic sampling with probability ∝ 1/s_{t,l}, `s_{t,l}` recomputed fresh each round with no smoothing, recycled-delta aggregation (`x_{t+1}=x_t+Δ̂_t`), and uplink/downlink (only non-recycled layers uploaded; full model downloaded). `luar_num_recycle_layers` (paper's `δ`) is dataset/model-specific in the paper (2–30 depending on benchmark), so the default of 1 is an unvalidated placeholder, not a paper-sourced value.
