...
† TSF adaptation applied: class-level assumptions replaced with time-series equivalents. Please use with caution.
  ...
  - **Caesar†**: KL-divergence term in importance estimation dropped (no class labels in TSF). Upload ranking uses participant subset only (server lacks sample counts for all clients), so ranks are compressed vs. the paper's global ranking.
