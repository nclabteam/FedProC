...
def _conv1d_fft(f, g, dim=-1):
    N = f.size(dim)
    M = g.size(dim)
    fast_len = next_fast_len(N + M - 1)
    F_f = fft.rfft(f, fast_len, dim=dim)
    F_g = fft.rfft(g, fast_len, dim=dim)
    out = fft.irfft(F_f * F_g.conj(), fast_len, dim=dim)
...
class ExponentialSmoothing(nn.Module):
    ...
    def forward(self, values, aux_values=None):
        ...
        init_weight, weight = self._get_exponential_weight(t)
        output = _conv1d_fft(self.dropout(values), weight, dim=1)
        output = init_weight * self.v0 + output
        if aux_values is not None:
            aux_weight = weight / (1 - self.weight) * self.weight
            aux_output = _conv1d_fft(self.aux_dropout(aux_values), aux_weight, dim=1)
            output = output + aux_output
        return output

    def _get_exponential_weight(self, T):
...
class GrowthLayer(nn.Module):
    def __init__(self, d_model, nhead, dropout=0.1):
        ...
        self.nhead = nhead
        self.z0 = nn.Parameter(torch.randn(nhead, self.d_head))
        self.in_proj = nn.Linear(d_model, d_model)
        self.es = ExponentialSmoothing(self.d_head, nhead, dropout=dropout)
...
class FourierLayer(nn.Module):
    ...
    def _extrapolate(self, x_freq, f, t):
        x_freq = torch.cat([x_freq, x_freq.conj()], dim=1)
        f = torch.cat([f, -f], dim=1)
        t_val = rearrange(
            torch.arange(t + self.pred_len, dtype=torch.float, device=x_freq.device),
            "t -> () () t ()",
        )
...
class LevelLayer(nn.Module):
    def __init__(self, d_model, c_out, dropout=0.1):
        super().__init__()
        self.c_out = c_out
        self.es = ExponentialSmoothing(1, c_out, dropout=dropout, aux=True)
        self.growth_pred = nn.Linear(d_model, c_out)
        self.season_pred = nn.Linear(d_model, c_out)
    ...
    def forward(self, level, growth, season):
        ...
        return rearrange(out, "b t h d -> b t (h d)")
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, res, level, attn_mask=None):
        season = self.dropout2(self.seasonal_layer(res))
        res = res - season[:, : -self.pred_len]
        growth = self.dropout1(self.growth_layer(res))
...
class DampingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        damping_factors = (self.damping_factor ** powers.view(-1, 1)).cumsum(dim=0)
...
class Decoder(nn.Module):
    def __init__(self, layers):
        super().__init__()
        self.layers = nn.ModuleList(layers)
        self.pred = nn.Linear(layers[0].d_model, layers[0].c_out)

    def forward(self, growths, seasons):
        growth_repr, season_repr = [], []
        for i, layer in enumerate(self.layers):
            g, s = layer(growths[i], seasons[i])
            growth_repr.append(g)
            season_repr.append(s)
        return self.pred(sum(growth_repr)), self.pred(sum(season_repr))
