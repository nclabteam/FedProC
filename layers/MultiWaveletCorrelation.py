...
from layers.WaveletUtils import get_filter
...
class FourierCrossAttentionW(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        xqkvw = xqkv_ft
        out_ft = torch.zeros(
            B, H, E, L // 2 + 1, device=xq.device, dtype=torch.cfloat
        )
        for i, j in enumerate(self.index_q):
            out_ft[:, :, :, j] = xqkvw[:, :, :, i]
...
class sparseKernelFT1d(nn.Module):
    def __init__(self, k, alpha, c=1, **kwargs):
        super().__init__()
        self.modes1 = alpha
...
class MWT_CZ1d(nn.Module):
    def __init__(self, k=3, alpha=64, L=0, c=1, base="legendre", **kwargs):
        super().__init__()
        self.k = k
        self.L = L
...
class MultiWaveletTransform(nn.Module):
    ...
    def __init__(
        self, ich=1, k=8, alpha=16, c=128, nCZ=1, L=0, base="legendre", **kwargs
    ):
        super().__init__()
        self.k = k
        self.c = c
        ...
        self.MWT_CZ = nn.ModuleList(
            MWT_CZ1d(k, alpha, L, c, base) for _ in range(nCZ)
        )
...
class MultiWaveletCross(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attn1 = FourierCrossAttentionW(
            in_channels, out_channels, seq_len_q, seq_len_kv, modes, activation, mode_select_method
        )
    ...
    def forward(self, q, k, v, mask=None):
        ...
        Ud_q: List[Tuple[Tensor, ...]] = []
        Ud_k: List[Tuple[Tensor, ...]] = []
        Ud_v: List[Tuple[Tensor, ...]] = []
        ...
        for i in range(ns - self.L):
            ...
            Us.append(self.attn3(sq, sk, sv, mask)[0])
